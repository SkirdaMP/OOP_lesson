...
Ранее обсуждалось три типа связи между объектами: *ассоциация*, *композиция* и *агрегация*. Но мы не сможем в полной мере воспользоваться мощью ООП без ещё одного важного типа связи - *наследования*.
**Наследование**  очень похоже на семейное древо: сын наследует фамилию отца и отчество от него. Так же могут перейти и черты характера.

Т.е ***наследование*** это тип взаимосвязи, когда дочерний класс имеет те же атрибуты и методы, что и родительский. Т.е он их **наследует**.

Так же дочерние классы могут **переопределять** методы родительских классов, чтобы более приспособить их к текущим требованиям задачи.

Так же родительский класс может быть *абстрактным*. Абстрактный класс - это как шаблон. Он не реализует поведение методов, он только сообщает дочерним классам, какие методы им нужно реализовать.

### Полиморфизм

**Полиморфизм** - это способность по-разному относиться к классу в зависимости от того, какой подкласс реализован. 

Т.е. это различная реализация класса родителя в наследниках. Например мы можем складывать числа, а можем строки, можем списки, а можем вообще объекты. Т.е знак "+" будет полиморфен, т.е метод реализации сложения будет иметь свою реализацию для разных типов объектов.

### Множественное наследование

Когда класс наследуется от двух и более родительских классов, тогда это называют множественным наследованием. Но это нужно использовать *аккуратно*, т.к если в двух классах реализован метод с одинаковым названием, реализуемый класс может не понять, каким из двух методов в разных родительских классах пользоваться. Множественное наследование лучше использовать, когда в одном классе должно объединиться поведение совсем различных родительских классов
